...
async function jsonFetch<T>(url: string, init?: RequestInit): Promise<T> {
  ...
  if (!response.ok) throw new Error(json.error || "Erreur rÃ©seau");
  return json as T;
}
...
function MarketDataLibrary({
  ...
  const createInstrument = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage("Instrument crÃ©Ã©. Vous pouvez maintenant importer ses bougies M1.");
      setTab("csv");
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Impossible de crÃ©er lâ€™instrument");
    } finally {
      setBusy(false);
    }
  };
  ...
  const uploadCsv = async (file: File) => {
    if (!importInstrumentId) {
      setMessage("SÃ©lectionnez dâ€™abord un instrument.");
      return;
    }
    ...
    try {
      ...
      setMessage(`${response.accepted} bougies validÃ©es Â· ${response.rejected} rejetÃ©es Â· ${response.deduplicated} doublons fusionnÃ©s Â· ${response.gaps.length} gap(s) dÃ©tectÃ©(s).`);
      onImported();
    } catch (error) {
      ...
      <button onClick={() => setOpen((value) => !value)} className="flex w-full items-center justify-between gap-4 p-5 text-left hover:bg-white/[0.02]">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[12px] text-mut">M1 canonique Â· validation OHLCV Â· dÃ©duplication Â· gaps Â· rÃ©tention 15 ans</p>
          </div>
        </div>
        ...
        <div className="border-t border-line p-5">
          ...
          {tab === "instrument" ? (
            <form onSubmit={createInstrument} className="grid gap-3 sm:grid-cols-3">
              ...
              <label><span className="yj-label mb-1 block">Nom affichÃ©</span><input className="yj-input" placeholder="U.S. Dollar Index" value={instrumentForm.displayName} onChange={(e) => setInstrumentForm({ ...instrumentForm, displayName: e.target.value })} /></label>
              <label><span className="yj-label mb-1 block">{d.assetClass}</span><select className="yj-select" value={instrumentForm.assetClass} onChange={(e) => setInstrumentForm({ ...instrumentForm, assetClass: e.target.value })}><option value="forex">Forex</option><option value="metal">MÃ©tal</option><option value="crypto">Crypto</option><option value="index">Indice</option><option value="commodity">MatiÃ¨re premiÃ¨re</option><option value="equity">Action</option></select></label>
              ...
              <label className="sm:col-span-2"><span className="yj-label mb-1 block">{d.quality}</span><input className="yj-input" placeholder="Source, couverture, horaires, contraintes de licenceâ€¦" value={instrumentForm.qualityNotes} onChange={(e) => setInstrumentForm({ ...instrumentForm, qualityNotes: e.target.value })} /></label>
              <div className="sm:col-span-3 flex justify-end"><button disabled={busy} className="yj-btn yj-btn-primary"><Plus size={15} />{busy ? "CrÃ©ationâ€¦" : d.createInstrument}</button></div>
            </form>
          ) : (
            <div className="grid gap-4 sm:grid-cols-[1fr_auto] sm:items-end">
              <label><span className="yj-label mb-1 block">{d.instruments}</span><select className="yj-select" value={importInstrumentId} onChange={(e) => setImportInstrumentId(e.target.value)}><option value="">SÃ©lectionner un instrument</option>{instruments.map((item) => <option key={item.id} value={item.id}>{item.symbol} â€” {item.displayName}</option>)}</select></label>
              <button disabled={busy || !instruments.length} onClick={() => fileRef.current?.click()} className="yj-btn yj-btn-primary"><FileUp size={16} />{busy ? "Importâ€¦" : d.importCsv}</button>
              <input ref={fileRef} hidden type="file" accept=".csv,text/csv" onChange={(e) => e.target.files?.[0] && uploadCsv(e.target.files[0])} />
              <p className="sm:col-span-2 text-[11.5px] leading-relaxed text-mut">Colonnes requises : <span className="mono text-white">timestamp, open, high, low, close</span> ; <span className="mono text-white">volume</span> est optionnel. Dates UTC ISO recommandÃ©es. Import fractionnÃ© Ã  2 000 M1 par lot pour protÃ©ger la base et le navigateur.</p>
            </div>
          )}

          {message && <p className={clsx("mt-4 rounded-xl border px-3 py-2 text-[12px]", message.includes("validÃ©es") || message.includes("crÃ©Ã©") ? "border-up/30 bg-up/10 text-up" : "border-down/30 bg-down/10 text-down")}>{message}</p>}
          <p className="mt-4 flex items-start gap-2 rounded-xl border border-gold/20 bg-gold/[0.05] px-3 py-2 text-[11.5px] leading-relaxed text-mut"><ShieldCheck size={15} className="mt-0.5 shrink-0 text-gold" />{d.licenseRequired}. Lâ€™import CSV suppose que lâ€™opÃ©rateur dÃ©tient les droits de stockage et dâ€™usage interne.</p>
        </div>
      )}
    </div>
  );
...
export function MarketReplayTab({ d, lang, accounts }: { d: Dict; lang: Lang; accounts: Account[] }) {
  ...
  const advance = useCallback(async (steps = 1) => {
    ...
    } catch (e) { setPlaying(false); setError(e instanceof Error ? e.message : "AvancÃ©e impossible"); } finally { setBusy(false); }
  }, [activeId, refreshSnapshot]);
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-5 xl:grid-cols-[360px_1fr]">
        <form onSubmit={createReplay} className="yj-card p-5 space-y-4">
          <div><h3 className="text-[15px] font-bold text-white">{d.startReplay}</h3><p className="mt-1 text-[12px] text-mut">SÃ©lectionnez un point de dÃ©part couvert par vos donnÃ©es rÃ©elles importÃ©es.</p></div>
          <label className="block"><span className="yj-label mb-1 block">{d.instruments}</span><select required className="yj-select" value={form.instrumentId} onChange={(e) => setForm({ ...form, instrumentId: e.target.value })}><option value="">â€”</option>{instruments.map((item) => <option key={item.id} value={item.id}>{item.symbol} â€” {item.displayName}</option>)}</select></label>
          <label className="block"><span className="yj-label mb-1 block">{d.account}</span><select className="yj-select" value={form.accountId} onChange={(e) => setForm({ ...form, accountId: e.target.value, initialCapital: accounts.find((item) => item.id === e.target.value)?.initialBalance ?? form.initialCapital })}><option value="">Capital personnalisÃ©</option>{accounts.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
          <label className="block"><span className="yj-label mb-1 block">Date / heure de dÃ©part (UTC)</span><input required type="datetime-local" className="yj-input" value={form.startAt} onChange={(e) => setForm({ ...form, startAt: e.target.value })} /></label>
          ...
          <button disabled={busy || !form.instrumentId} className="yj-btn yj-btn-primary w-full"><RotateCcw size={16} />{busy ? "CrÃ©ationâ€¦" : d.startReplay}</button>
        </form>

        <div className="yj-card min-w-0 p-5">
          {!snapshot ? (
            <div className="flex min-h-[360px] flex-col items-center justify-center text-center"><RotateCcw size={38} className="mb-3 text-gold/70" /><p className="font-bold text-white">SÃ©lectionnez ou crÃ©ez une session Replay</p><p className="mt-1 max-w-md text-[12.5px] text-mut">Les donnÃ©es ne sont jamais simulÃ©es. Importez des M1 rÃ©elles, puis choisissez un dÃ©part historique.</p></div>
          ) : (
            <>
              <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2"><select className="yj-select !w-48 !py-1.5 text-[12px]" value={activeId} onChange={(e) => { setPlaying(false); setActiveId(e.target.value); }}><option value="">Session Replay</option>{sessions.map((session) => <option key={session.id} value={session.id}>{instruments.find((i) => i.id === session.instrumentId)?.symbol ?? "â€”"} Â· {new Date(session.startAt).toLocaleDateString()}</option>)}</select><span className="yj-badge yj-badge-gold">{activeInstrument?.symbol ?? "â€”"} Â· {snapshot.session.timeframe}</span></div>
              </div>
      ...
      {snapshot && (
        <div className="grid gap-5 xl:grid-cols-[1fr_340px]">
          <div className="yj-card p-5"><div className="mb-4 flex items-center justify-between"><div><h3 className="text-[15px] font-bold text-white">Simulation de position</h3><p className="text-[12px] text-mut">EntrÃ©e au dernier close visible ; SL prioritaire si SL et TP touchent la mÃªme bougie.</p></div><span className="yj-badge yj-badge-gold">{d.replayMode}</span></div><div className="grid gap-3 md:grid-cols-4"><label><span className="yj-label mb-1 block">{d.stopLoss}</span><input type="number" step="any" className="yj-input mono" placeholder="Optionnel" value={order.stopLoss} onChange={(e) => setOrder({ ...order, stopLoss: e.target.value })} /></label><label><span className="yj-label mb-1 block">{d.takeProfit}</span><input type="number" step="any" className="yj-input mono" placeholder="Optionnel" value={order.takeProfit} onChange={(e) => setOrder({ ...order, takeProfit: e.target.value })} /></label><label><span className="yj-label mb-1 block">{d.positionSize}</span><input type="number" min="0.0001" step="any" className="yj-input mono" value={order.quantity} onChange={(e) => setOrder({ ...order, quantity: e.target.value })} /></label><label><span className="yj-label mb-1 block">Commission</span><input type="number" min="0" step="any" className="yj-input mono" value={order.commission} onChange={(e) => setOrder({ ...order, commission: e.target.value })} /></label></div><div className="mt-4 flex gap-2"><button className="yj-btn bg-up/15 text-up border border-up/30 hover:bg-up/25" disabled={busy} onClick={() => void sendOrder("buy")}><TrendingUp size={16} />{d.buy}</button><button className="yj-btn bg-down/15 text-down border border-down/30 hover:bg-down/25" disabled={busy} onClick={() => void sendOrder("sell")}><TrendingDown size={16} />{d.sell}</button></div></div>
          <div className="yj-card p-5"><h3 className="text-[15px] font-bold text-white">{d.openPositions}</h3><div className="mt-3 space-y-2">{openPositions.length ? openPositions.map((position) => <div key={position.id} className="rounded-xl border border-line p-3"><div className="flex items-center justify-between"><span className={clsx("font-bold text-[12px]", position.direction === "long" ? "text-up" : "text-down")}>{position.direction.toUpperCase()} Â· {position.quantity}</span><button className="text-[11px] font-bold text-gold hover:underline" onClick={() => void sendOrder("close", position.id)}>{d.closePosition}</button></div><p className="mono mt-1 text-[11px] text-mut">E {position.entryPrice} Â· SL {position.stopLoss ?? "â€”"} Â· TP {position.takeProfit ?? "â€”"}</p></div>) : <p className="py-6 text-center text-[12px] text-mut">Aucune position ouverte</p>}</div></div>
        </div>
      )}

      {gaps.length > 0 && <div className="rounded-2xl border border-gold/25 bg-gold/[0.06] p-4 text-[12.5px] text-gold"><div className="flex items-center gap-2 font-bold"><AlertTriangle size={16} />{gaps.length} gap(s) M1 dÃ©tectÃ©(s)</div><p className="mt-1 text-mut">Les gaps restent visibles dans le moteur jusquâ€™Ã  leur rÃ©cupÃ©ration via un import de correction validÃ© ; aucune bougie nâ€™est inventÃ©e.</p></div>}
...
export function BacktestingTab({ d, lang, accounts, liveTrades }: { d: Dict; lang: Lang; accounts: Account[]; liveTrades: Trade[] }) {
  ...
  const refresh = useCallback(async () => {
    ...
    } catch { /* comparaison Replay optionnelle : on garde l'espace rÃ©servÃ© en cas d'Ã©chec */ }
  }, []);
  useEffect(() => {
    const timer = setTimeout(() => {
      void refresh();
  ...
  const createStrategy = async () => {
    ...
    try {
      const strategy = await jsonFetch<Strategy>("/api/strategies", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: strategyName.trim(), description: "Signal momentum de la bougie prÃ©cÃ©dente", rules: { entryRule: "previous_candle_momentum", stopPct: 0.25, targetRR: 2 }, newsRules: { enabled: true, minutesBefore: 15, minutesAfter: 15, minimumImportance: "high" } }) });
      setStrategies((items) => [...items, strategy]); setForm((f) => ({ ...f, strategyId: strategy.id })); setStrategyName("");
    } catch (e) { setError(e instanceof Error ? e.message : "StratÃ©gie impossible"); }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4"><div><div className="flex items-center gap-2"><span className="yj-badge yj-badge-up">{d.backtestMode}</span><h2 className="text-2xl font-extrabold tracking-tight text-white">{d.backtesting}</h2></div><p className="mt-1 text-[13px] text-mut">ExÃ©cution sÃ©quentielle bougie par bougie avec moteur de position partagÃ© et rÃ¨gles news.</p></div><span className="flex items-center gap-2 rounded-xl border border-up/20 bg-up/[0.06] px-3 py-2 text-[11.5px] font-semibold text-up"><ShieldCheck size={15} />{d.antiFutureLeak}</span></div>

      <div className="grid gap-5 xl:grid-cols-[390px_1fr]">
        <form onSubmit={run} className="yj-card p-5 space-y-4"><div><h3 className="text-[15px] font-bold text-white">{d.createBacktest}</h3><p className="mt-1 text-[12px] text-mut">Le moteur ne consomme que les bougies importÃ©es de la pÃ©riode sÃ©lectionnÃ©e.</p></div><label><span className="yj-label mb-1 block">Nom</span><input required className="yj-input" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} /></label><label><span className="yj-label mb-1 block">{d.instruments}</span><select required className="yj-select" value={form.instrumentId} onChange={(e) => setForm({ ...form, instrumentId: e.target.value })}><option value="">â€”</option>{instruments.map((item) => <option key={item.id} value={item.id}>{item.symbol} â€” {item.displayName}</option>)}</select></label><div className="grid grid-cols-2 gap-3"><label><span className="yj-label mb-1 block">DÃ©but UTC</span><input required type="datetime-local" className="yj-input" value={form.startAt} onChange={(e) => setForm({ ...form, startAt: e.target.value })} /></label><label><span className="yj-label mb-1 block">Fin UTC</span><input required type="datetime-local" className="yj-input" value={form.endAt} onChange={(e) => setForm({ ...form, endAt: e.target.value })} /></label></div><div className="grid grid-cols-3 gap-3"><label><span className="yj-label mb-1 block">TF</span><select className="yj-select" value={form.timeframe} onChange={(e) => setForm({ ...form, timeframe: e.target.value })}>{frameOptions.map((frame) => <option key={frame}>{frame}</option>)}</select></label><label><span className="yj-label mb-1 block">Capital</span><input type="number" className="yj-input mono" value={form.initialCapital} onChange={(e) => setForm({ ...form, initialCapital: Number(e.target.value) })} /></label><label><span className="yj-label mb-1 block">{d.risk}</span><input type="number" min="0" step="0.1" className="yj-input mono" value={form.riskPct} onChange={(e) => setForm({ ...form, riskPct: Number(e.target.value) })} /></label></div><label><span className="yj-label mb-1 block">StratÃ©gie</span><select className="yj-select" value={form.strategyId} onChange={(e) => setForm({ ...form, strategyId: e.target.value })}><option value="">Candle Momentum (dÃ©faut)</option>{strategies.map((strategy) => <option key={strategy.id} value={strategy.id}>{strategy.name}</option>)}</select></label><label className="flex items-center gap-2 rounded-xl border border-line bg-white/[0.02] p-3 text-[12.5px] text-mut"><input type="checkbox" checked={form.useNewsFilter} onChange={(e) => setForm({ ...form, useNewsFilter: e.target.checked })} /> Bloquer les entrÃ©es Â±15 min autour des news Ã  forte importance</label><button disabled={busy || !form.instrumentId} className="yj-btn yj-btn-primary w-full"><FlaskConical size={16} />{busy ? "Calculâ€¦" : d.runBacktest}</button></form>

        <div className="space-y-5"><div className="yj-card p-5"><div className="mb-4 flex flex-wrap items-center justify-between gap-3"><div><h3 className="text-[15px] font-bold text-white">{d.completedRuns}</h3><p className="text-[12px] text-mut">Les rÃ©sultats Backtest restent sÃ©parÃ©s du Journal Live.</p></div><div className="flex gap-2"><input className="yj-input !w-40 !py-1.5" placeholder="Nouvelle stratÃ©gie" value={strategyName} onChange={(e) => setStrategyName(e.target.value)} /><button className="yj-btn yj-btn-ghost !py-1.5" type="button" onClick={createStrategy}><Plus size={14} />StratÃ©gie</button></div></div>{runs.length ? <div className="overflow-x-auto"><table className="yj-table w-full"><thead><tr><th>Nom</th><th>TF</th><th>PÃ©riode</th><th>Statut</th><th></th></tr></thead><tbody>{runs.map((run) => <tr key={run.id} className="cursor-pointer" onClick={() => void loadRun(run.id)}><td className="font-bold text-white">{run.name}</td><td className="mono text-mut">{run.timeframe}</td><td className="mono text-[11px] text-mut">{new Date(run.startAt).toLocaleDateString()} â†’ {new Date(run.endAt).toLocaleDateString()}</td><td><span className={clsx("yj-badge", run.status === "completed" ? "yj-badge-up" : run.status === "failed" ? "yj-badge-down" : "yj-badge-gold")}>{run.status}</span></td><td className="text-gold">Voir â†’</td></tr>)}</tbody></table></div> : <p className="py-8 text-center text-[12.5px] text-mut">Aucun Backtest exÃ©cutÃ©.</p>}</div>
      ...
      <div className="yj-card p-6"><div className="mb-4 flex items-center gap-2"><BarChart3 size={18} className="text-gold" /><h3 className="text-[15px] font-bold text-white">Comparaison Live / Replay / Backtest</h3></div><div className="overflow-x-auto"><table className="yj-table w-full"><thead><tr><th>Mode</th><th className="!text-right">Trades</th><th className="!text-right">Win Rate</th><th className="!text-right">Profit Factor</th><th className="!text-right">Return R</th><th className="!text-right">PnL / Equity</th></tr></thead><tbody><tr><td><span className="yj-badge yj-badge-up">{d.liveMode}</span></td><td className="mono !text-right">{live.n}</td><td className="mono !text-right">{live.winRate.toFixed(1)}%</td><td className="mono !text-right">{live.pf === Infinity ? "âˆž" : live.pf.toFixed(2)}</td><td className="mono !text-right">{fmtR(live.totalR)}</td><td className={clsx("mono !text-right font-bold", live.net >= 0 ? "text-up" : "text-down")}>{fmtMoney(live.net, "USD", lang)}</td></tr><tr><td><span className="yj-badge yj-badge-gold">{d.replayMode}</span></td>{replayCompare && replayCompare.count > 0 ? (<><td className="mono !text-right">{replayCompare.n}</td><td className="mono !text-right">{replayCompare.winRate.toFixed(1)}%</td><td className="mono !text-right">{replayCompare.pf === Infinity ? "âˆž" : replayCompare.pf.toFixed(2)}</td><td className="mono !text-right">{fmtR(replayCompare.totalR)}</td><td className={clsx("mono !text-right font-bold", replayCompare.net >= 0 ? "text-up" : "text-down")}>{fmtMoney(replayCompare.net, "USD", lang)}</td></>) : (<><td className="mono !text-right">â€”</td><td className="mono !text-right">â€”</td><td className="mono !text-right">â€”</td><td className="mono !text-right">â€”</td><td className="text-right text-mut">Aucune position clÃ´turÃ©e en Replay</td></>)}</tr><tr><td><span className="yj-badge yj-badge-mut">{d.backtestMode}</span></td><td className="mono !text-right">{comparisonSummary.trades}</td><td className="mono !text-right">{comparisonSummary.winRate.toFixed(1)}%</td><td className="mono !text-right">{comparisonSummary.profitFactor === null ? "âˆž" : comparisonSummary.profitFactor.toFixed(2)}</td><td className="mono !text-right">{fmtR(comparisonSummary.totalR)}</td><td className={clsx("mono !text-right font-bold", comparisonSummary.net >= 0 ? "text-up" : "text-down")}>{fmtMoney(comparisonSummary.net, "USD", lang)}</td></tr></tbody></table></div></div>
      {error && <p className="rounded-xl border border-down/30 bg-down/10 px-3 py-2 text-[12px] text-down">{error}</p>}
    </div>
  );
}
...
function BacktestResult({ summary, lang }: { summary: ReturnType<typeof safeSummary>; lang: Lang }) {
  ...
  return <div className="yj-card p-5"><div className="mb-4 flex items-center justify-between"><div><h3 className="text-[15px] font-bold text-white">RÃ©sultat Backtest</h3><p className="text-[12px] text-mut">Moteur sÃ©quentiel Â· exÃ©cution conservatrice intrabar.</p></div><span className="mono text-[15px] font-bold text-white">Equity {fmtMoney(summary.finalEquity, "USD", lang)}</span></div><div className="grid grid-cols-2 gap-3 sm:grid-cols-5"><MiniMetric label="Trades" value={String(summary.trades)} /><MiniMetric label="Win Rate" value={`${summary.winRate.toFixed(1)}%`} tone="up" /><MiniMetric label="PF" value={summary.profitFactor === null ? "âˆž" : summary.profitFactor.toFixed(2)} tone="gold" /><MiniMetric label="Return R" value={fmtR(summary.totalR)} tone={summary.totalR >= 0 ? "up" : "down"} /><MiniMetric label="Max DD" value={fmtMoney(-summary.maxDrawdown, "USD", lang)} tone="down" /></div>{equity.length ? <div className="mt-5"><EquityChart data={equity} xLabel="Temps de backtest" yLabel="Equity ($)" currency="$" /></div> : <p className="mt-5 text-center text-[12px] text-mut">Aucune opÃ©ration gÃ©nÃ©rÃ©e par la pÃ©riode/rÃ¨gle sÃ©lectionnÃ©e.</p>}</div>;
}
...
export function EconomicCalendarTab({ d, lang }: { d: Dict; lang: Lang }) {
  ...
  const addEvent = async (e: React.FormEvent) => {
    ...
    } catch (e) { setError(e instanceof Error ? e.message : "Ã‰vÃ©nement impossible"); } finally { setBusy(false); }
  };
  ...
  return <div className="space-y-6"><div className="flex flex-wrap items-start justify-between gap-4"><div><div className="flex items-center gap-2"><span className="yj-badge yj-badge-gold">MACRO</span><h2 className="text-2xl font-extrabold tracking-tight text-white">{d.economicCalendar}</h2></div><p className="mt-1 text-[13px] text-mut">Historique et Ã  venir Â· Actual / Forecast / Previous Â· rÃ©visions et visibilitÃ© temporelle.</p></div><button className="yj-btn yj-btn-primary" onClick={() => setShowAdd((value) => !value)}><Plus size={16} />{d.addEconomicEvent}</button></div>
    ...
    <div className="yj-card overflow-hidden"><div className="overflow-x-auto"><table className="yj-table w-full"><thead><tr><th>Date UTC</th><th>{d.currency}</th><th>{d.country}</th><th>Ã‰vÃ©nement</th><th>{d.category}</th><th>{d.importance}</th><th className="!text-right">{d.previous}</th><th className="!text-right">{d.forecast}</th><th className="!text-right">{d.actual}</th><th>Statut</th></tr></thead><tbody>{events.length ? events.map((event) => { const released = event.status === "released" || event.status === "revised"; return <tr key={event.id}><td className="mono text-[11.5px] text-mut">{new Date(event.scheduledAt).toLocaleString(lang === "fr" ? "fr-FR" : "en-GB", { timeZone: "UTC", dateStyle: "short", timeStyle: "short" })}</td><td className="font-bold text-gold">{event.currency || "â€”"}</td><td className="text-mut">{event.country || "â€”"}</td><td className="font-semibold text-white max-w-[260px] truncate">{event.title}</td><td className="text-mut">{event.category || "â€”"}</td><td><span className={clsx("yj-badge", importantClass(event.importance))}>{event.importance.toUpperCase()}</span></td><td className="mono !text-right text-mut">{event.previous ?? "â€”"}</td><td className="mono !text-right text-white">{event.forecast ?? "â€”"}</td><td className={clsx("mono !text-right font-bold", released ? "text-white" : "text-mut")}>{released ? event.actual ?? "â€”" : "â€”"}</td><td><span className={clsx("yj-badge", released ? "yj-badge-up" : "yj-badge-gold")}>{released ? d.published : d.scheduled}</span></td></tr>; }) : <tr><td colSpan={10} className="!py-14 text-center text-mut">Aucun Ã©vÃ©nement dans la pÃ©riode sÃ©lectionnÃ©e.</td></tr>}</tbody></table></div></div>
    <div className="flex items-start gap-2 rounded-xl border border-gold/20 bg-gold/[0.05] p-3 text-[11.5px] leading-relaxed text-mut"><ShieldCheck size={15} className="mt-0.5 shrink-0 text-gold" />Les champs Actual et rÃ©visions ne deviennent visibles dans un Replay/Backtest quâ€™aprÃ¨s leur timestamp de publication/connaissance historique.</div>
    {error && <p className="rounded-xl border border-down/30 bg-down/10 px-3 py-2 text-[12px] text-down">{error}</p>}
  </div>;
}
