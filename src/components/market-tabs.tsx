...
export function MarketReplayTab({ d, lang, accounts }: { d: Dict; lang: Lang; accounts: Account[] }) {
  ...
  const [form, setForm] = useState({ instrumentId: "", accountId: accounts[0]?.id ?? "", startAt: asDatetimeInput(new Date(Date.now() - 24 * 60 * 60_000)), timeframe: "15m", initialCapital: accounts[0]?.initialBalance ?? 10000, speed: 1 });
  ...
  useEffect(() => { void refreshInstruments(); void refreshSessions(); }, [refreshInstruments, refreshSessions]);
  useEffect(() => { void refreshSnapshot(); }, [activeId, refreshSnapshot]);
...
export function BacktestingTab({ d, lang, accounts, liveTrades }: { d: Dict; lang: Lang; accounts: Account[]; liveTrades: Trade[] }) {
  ...
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [form, setForm] = useState({ name: "Candle Momentum", instrumentId: "", accountId: accounts[0]?.id ?? "", strategyId: "", timeframe: "15m", startAt: asDatetimeInput(new Date(Date.now() - 30 * 86400_000)), endAt: asDatetimeInput(new Date(Date.now() - 86400_000)), initialCapital: accounts[0]?.initialBalance ?? 10000, riskPct: 1, commissionPerOrder: 0, useNewsFilter: true });
...
export function EconomicCalendarTab({ d, lang }: { d: Dict; lang: Lang }) {
  ...
  const [error, setError] = useState<string | null>(null);
  const [form, setForm] = useState({ title: "", currency: "USD", country: "US", category: "", importance: "high", scheduledAt: asDatetimeInput(new Date(Date.now() + 3600_000)), knownAt: asDatetimeInput(new Date()), publishedAt: "", previous: "", forecast: "", actual: "", sourceUrl: "" });
