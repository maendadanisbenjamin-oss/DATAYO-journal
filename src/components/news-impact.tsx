  ...
  return (
    <div className="rounded-xl border border-line bg-white/[0.02] p-3">
      ...
      <div className="mt-2 grid grid-cols-2 gap-x-3 gap-y-1 text-[11px]">
        ...
        <span className="text-mut">Profit Factor</span><span className="mono text-right text-gold">{metric.profitFactor === null ? "âˆž" : metric.profitFactor.toFixed(2)}</span>
  ...
  return (
    <section className="yj-card p-6">
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3"><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-gold/15 text-gold"><Newspaper size={18} /></span><div><h3 className="text-[15px] font-bold text-white">Impact des actualitÃ©s Ã©conomiques</h3><p className="text-[12px] text-mut">CalculÃ© Ã  partir des Ã©vÃ©nements associÃ©s automatiquement aux entrÃ©es de trades horodatÃ©es.</p></div></div>
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-3"><MetricCard title="Avant news" metric={value?.before ?? empty} lang={lang} /><MetricCard title="Pendant news" metric={value?.during ?? empty} lang={lang} /><MetricCard title="AprÃ¨s news" metric={value?.after ?? empty} lang={lang} /></div>
      <div className="mt-4"><p className="yj-label mb-2">Par importance</p><div className="grid gap-3 sm:grid-cols-3"><MetricCard title="Ã‰levÃ©e" metric={value?.byImportance.high ?? empty} lang={lang} /><MetricCard title="Moyenne" metric={value?.byImportance.medium ?? empty} lang={lang} /><MetricCard title="Faible" metric={value?.byImportance.low ?? empty} lang={lang} /></div></div>
    </section>
  );
}
